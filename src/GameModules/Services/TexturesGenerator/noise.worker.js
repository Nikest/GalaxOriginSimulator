...
function makePlanetTexture(props, push) {
    const noiseAlpha = new Noise({
        octaves: surfaceType().octaves,
        amplitude: surfaceType().amplitude,
        frequency: surfaceType().frequency,
        persistence: 0.6,
        min: 0,
        max: 255
    });
    ...
    const seaLevel = rand(80, 200);
    const seaColor = surfacePalette('sea');
    const rockyColor = surfacePalette('rocky');
    ...
    for (let y = 0; y < props.size; y += 1) for (let x = 0; x < props.size; x += 1) {
        ...
        if (n < seaLevel) {
            ...
            const ns = noiseShadow.spherical2D(props.size, x, y);

            if (pixel <= 90 ) {

                push(rockyColor[0].R - ns, count); count += 1;
                push(rockyColor[0].G - ns, count); count += 1;
                push(rockyColor[0].B - ns, count); count += 1;

                push(255, count); count += 1;

            } else if (pixel <= 120) {
                push(rockyColor[1].R- ns, count); count += 1;
                push(rockyColor[1].G- ns, count); count += 1;
                push(rockyColor[1].B- ns, count); count += 1;

                push(255, count); count += 1;

            } else if (pixel <= 170) {
                push(rockyColor[2].R- ns, count); count += 1;
                push(rockyColor[2].G- ns, count); count += 1;
                push(rockyColor[2].B- ns, count); count += 1;

                push(255, count); count += 1;

            } else {
                push(pixel, count); count += 1;
                push(pixel, count); count += 1;
                push(pixel, count); count += 1;
