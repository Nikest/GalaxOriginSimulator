...
export const astroWorker = {
    ...
    getPlanetsTemplateCount(mapFn) {
        ...
        return Array(templateCount).fill(0).map(mapFn)
    },
    ...
    generateOrbitsRadius(barycenter: Barycenter) {
        ...
        const firstOrbitRadius =
            barycenter.centralBody.radius * (barycenter.centralBody.class === 'planet' ? randF / 10 : randF);

        barycenter.orbits.forEach((o, i) => {
            const radius = (i === 0) ?
                firstOrbitRadius : (barycenter.orbits[i - 1].selfOrbit.A * (rand(170, 198) / 100));
